...
In the source, `{{BASH}}` / `{{READ}}` / `{{ASK}}` … expand to the host's tool name (an undeclared
variable is a build error), and `<!--only:claude-code--> … <!--/only-->` includes a block for one
frontend only — use it when the *guidance* differs, not when only a tool name does.
