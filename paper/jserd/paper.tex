...
checks and evaluation against itself. The assessments are the authors',
...
Qualitative assessment of the authors from the cited sources: see the text
for the operationalization of each dimension.}
\centering
...
the Claude Code skills by a build script. The script measures the shared
...
lines are the count the authors observed between the two hand-maintained
...
means failure: a rejected cycle, a REFUTED verdict, an audit with any phase
not graded followed (a fake, a skip, or a degraded phase), or a partial
skill installation. Exit code 2 means invalid CLI
...
same holds for gates. Without the cycle, the median
...
defaults) and one verify section as faked: in t05 the session declared the
success criterion met while all three objective checks were false, though
its notes attribute the failures to pre-existing conditions outside the
change. The grade is deliberately coarse: a verify section that declares
success while every objective check is false is flagged without reading the
context, so the t05 grade names a risk the session had documented rather
than a fabrication --- the notes are what a human reads to tell the two
apart. The audit names the risk; the 24 session task files are archived
under
