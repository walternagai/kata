...
| **Fonte única das fases (`phases/*.md`)** | `phases/kata-<fase>.md` gera tanto `opencode/` quanto `claude-code/` via `make build-skills` | Elimina divergência entre frontends; 92,6% do conteúdo é compartilhado (`scripts/build_skills.py --stats`); `tests/test_skills_build.py` garante sincronia | Fable-skills, claude-skills-vault, ring são coleções manuais de skills — alto risco de drift entre versões/plataformas |
...
| **Schema `.kata/<task>.yaml` declarativo** | Configuração declarativa por tarefa (fit, think, verify, twins, artifact) versionada em YAML | Rastreia intent, auth, pending, twins por tarefa; auditável e diffável, não apenas conversa | Nenhum concorrente menciona schema declarativo de tarefa por arquivo |
| **Domain adapters gerados de fonte única** | `domains/kata-<domínio>.md` gera skills para OpenCode e Claude Code; 4 adapters disponíveis: `kata-devops`, `kata-data-analysis`, `kata-research`, `kata-docs` | Estende o ciclo a devops/infra, dados, pesquisa e docs sem duplicar textos entre frontends; domain skills são opcionais | Fable Method tem domain adapters manuais; outros competidores não têm mecanismo de geração compartilhada por domínio |
