...
---
<!-- Gerado por scripts/build_skills.py a partir de domains/kata-research.md. Não edite aqui. -->
...
3. **Estado atual** (`bash`): `git log`, `git status`, arquivos tocados
...
- **Comportamento**: reproduzir com um teste/script mínimo (`bash`)
- **Achado de revisão**: apontar arquivo:linha (`grep`)
...
2. Reproduzir comportamento em teste/script mínimo (`bash`) quando aplicável
