...
3. Rodar a verificação de schema/amostra do domínio ({{RUN}}):
...
4. Se a ação envolver dados sensíveis/restritos, obter AUTH com citação exata
   do usuário via {{ASK}} e registrar em `auth.action_taken: true`,
   `auth.authorized: true`, `auth.action` e `auth.quote`
5. Verificar o resultado real depois da ação ({{RUN}})
