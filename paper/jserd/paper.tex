...
Section~\ref{sec:impact} reports the research impact and evaluation.
...
followed, skipped, or faked, and names the concrete risk each skip or fake
created. The faked pattern (R7-1) is a phase marked as answered without real
...
scope creep for the earlier task. The remaining two scenarios expect
UNVERIFIABLE for confessed blind spots: a test in a language the judge has
no probes for (Elixir \texttt{.exs}), and a task that claims no
reproducible check at all.
