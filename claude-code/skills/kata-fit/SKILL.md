...
- **`AskUserQuestion`** (via `kata-question`): classifique a tarefa com o usuário, uma pergunta por vez.
...
rotas nomeadas — acima do limite de 4 opções do `AskUserQuestion` — prefira
listar as opções em texto e perguntar em texto livre, ou use
`AskUserQuestion` com as 4 rotas mais comuns e deixe `inference` acessível
via "Other":
