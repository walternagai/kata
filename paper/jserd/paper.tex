...
and honest work it must not accuse. Eleven scenarios plant a fraud that
...
five scenarios expect a non-REFUTED verdict --- four entirely honest tasks
that must come back VERIFIED and one (s06) whose debris must be found while
its look-alike names are spared (VERIFIED WITH CAVEATS) --- and three expect
...
The strongest external evidence is the executed comparison of
Section~\ref{sec:exec-comparison}: four gates reviewed the same 19
scenarios, and Kata was the only one with zero false positives and a
median review time of about one second. That comparison is a direct answer
to the journal's request for a software article to demonstrate its advance
by comparison with available related software, and its raw verdicts are
reproducible from the repository (\texttt{eval/compare\_gates.py},
\texttt{eval/results/compare\_gates\_round2.json}).
