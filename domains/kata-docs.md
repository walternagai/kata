...
4. **Estado** ({{RUN}}): `git log` do arquivo (o que mudou), `git status`
...
- **Comando citado**: rodar o comando e conferir que a saída bate ({{RUN}})
- **Flag/assinatura**: grep na fonte ({{SEARCH}})
...
2. Conferir cada comando/flag/número citado na fonte ({{SEARCH}}/{{RUN}})
...
- Não citar comando/flag sem abrir a fonte ({{READ}}/{{SEARCH}})
